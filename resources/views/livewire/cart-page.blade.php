          ...
          <table class="w-full">
            ...
            <tbody>
              ...
              <tr wire:key='{{ $item['product_id'] }}'>
                ...
                <td class="py-4">
                  {{ Number::currency($item['unit_amount'], 'KRW') }}
                </td>
                <td class="py-4">
                  <div class="flex items-center">
                ...
                <td class="py-4">
                  {{ Number::currency($item['total_amount'], 'KRW') }}
                </td>
                <td>
              ...
              @empty
              <tr>
                <td colspan="5" class="py-4 text-4xl font-semibold text-center text-slate-500">No item available in Cart!</td>
              </tr>
              @endforelse
              <!-- More product rows -->
            </tbody>
      ...
      <div class="md:w-1/4">
        <div class="p-6 bg-white rounded-lg shadow-md">
          ...
          <div class="flex justify-between mb-2">
            <span>Subtotal</span>
            <span>{{ Number::currency($grand_total, 'KRW') }}</span>
          </div>
          <div class="flex justify-between mb-2">
            <span>Taxes</span>
            <span>{{ Number::currency(0, 'KRW') }}</span>
          </div>
          <div class="flex justify-between mb-2">
            <span>Shipping</span>
            <span>{{ Number::currency(0, 'KRW') }}</span>
          ...
          <div class="flex justify-between mb-2">
            ...
            <span class="font-semibold">{{ Number::currency($grand_total, 'KRW') }}</span>
          </div>
          @if($cart_items)
            <a href="/checkout" class="text-center block w-full px-4 py-2 mt-4 text-white bg-blue-500 rounded-lg">Checkout</a>
          @endif
        </div>
      </div>
    </div>
